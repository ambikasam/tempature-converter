...
/*- (void)updateValues {
    if (([self.fahrenheitTextField.placeholder isEqualToString: self.selectedField])) {
        float celsius = [self farenheit2celsius:[self.fahrenheitTextField.text floatValue]];
        self.celsiusTextField.text = [NSString stringWithFormat:@"%.2f", celsius];
    }
    else if ([self.celsiusTextField.placeholder isEqualToString: self.selectedField])
    {
